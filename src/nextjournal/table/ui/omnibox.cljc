...
(defn id [opts & args]
  (->> (concat (:state/path-prefix opts) args)
       flatten
       (map name)
       (interpose "-")
       (apply str)))

(defn anchor-id [opts & args]
  (apply str "--" (apply id opts args)))
...
(defn popover [{:keys [choices] :as opts}]
  [:div.w-full {:id      (id opts :input :popover)
                :popover :manual
                :style   {:background      :white
                          :position        :fixed
                          :position-anchor (anchor-id opts :input)
