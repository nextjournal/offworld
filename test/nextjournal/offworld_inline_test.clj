...
(deftest inline-defers-a-closure-behind-a-token
  (let [ran (atom false)]
    (binding [inline/*conn-id* "conn-1"]
      (let [action    (inline/inline (reset! ran true))
            [k tok]   action]
...
(deftest a-derived-body-outlives-every-connection
  (reset! !outlives 0)
  (let [action (inline/inline (swap! !outlives inc))]
    (nxr/register-system->state! deref)
    (std/register-standard-nexus!)
...
(deftest a-captured-local-keeps-the-bodies-apart
  (let [seen (atom [])]
    (binding [inline/*conn-id* "conn-1"]
      (let [actions (doall (for [v [:a :b]] (inline/inline (swap! seen conj v))))
...
(deftest a-body-reaches-its-connection-without-capturing-it
  (reset! !per-conn {})
  (let [action  (inline/inline (swap! !per-conn update (inline/conn-id) (fnil inc 0)))
        [_ tok] action]
...
(deftest a-client-value-rides-beside-the-token
  (let [action      (inline/inline (reset! !slotted (inline/client! [:event.target/value])))
        [k tok ref] action]
    (is (= ::inline/invoke k))
...
(deftest a-hoisted-ref-may-mention-a-local
  (let [path        [:a :b]
        action      (inline/inline (reset! !slotted (inline/client! [::state path])))
        [_ tok ref] action]
    (is (inline/derived-token? tok)
...
(deftest a-marked-action-is-wrapped-into-one
  (let [action (inline/inline (swap! !shared inc))]
...
(deftest a-lone-action-keeps-its-modifiers
  (let [action (vary-meta (inline/inline (swap! !shared inc))
