...
#+begin_src clojure
;; an intent — the actions one click means, written where the click is
[:button {:on {:click [[::pan-to id 4.89 52.38]
                       [:effects/save path {:center [4.89 52.38]}]]}}
 "Amsterdam"]

;; each action, tagged with the world it runs in
(nxr/register-effect! ::pan-to      ^::🪐/client (fn [ctx _ id lng lat] ...))
(nxr/register-effect! :effects/save ^::🪐/server (fn [_ system path v] ...))
#+end_src

One click, two worlds, one vector — the map pans in the browser while
the new centre is saved on the server, and nothing in the intent names
a stage.
