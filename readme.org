...
see it everywhere. Reynolds (1972) described it as a compiler
...
Note that, this too, is a defunctionalization. We have done string
interpolation, but it stands as a single dumb transport, rather than
complex code generation. It puts an opaque blob in one fixed slot, beside a
dispatch URL which is constant for the whole app. The JavaScript around it is
byte-identical on every element.

Compare that against other datastar patterns, where each expression is
customized by hand (or codegen), each one inventing its own mini-language
for conveying values and procedure-calls across stage boundaries.
...
/Each registered nexus-item within an intent may run in a different stage/.
