...
*An intent is one per decision, not one per sample.* The decision here is not
whether Nexus sees a =pointermove= — it can — but whether the /server/ does.

In SSR every dispatch runs =divert=, and only a dispatch left holding server
actions becomes an HTTP request. So the rule that matters is that the per-frame
leg must be client-only. The demo's column resizer is the shape:
...
=::rz/preview= is client-marked and writes =gridTemplateColumns= directly, so a
frame's dispatch resolves entirely in the browser, =divert= returns nothing, and
the generated attribute — =_sp && @get(…)= — never makes the call. =::rz/commit=
is server-marked and sits behind a guard that only opens on =pointerup=. Sixty
dispatches a second, one request per gesture.

Note also which value crosses. The width isn't carried from the drag; it is read
back off the element at commit time by =[::rz/width grid-id idx]=, at the last
moment before the action leaves. The per-frame leg writes to the DOM and the
commit leg reads from it, so nothing has to accumulate drag state anywhere.

If even the client-side dispatch per frame is too much — and it is measurable
work, a transit decode and a full Nexus pass each time — the next move is to
give one client effect its own =requestAnimationFrame= loop, started by
=pointerdown= and torn down by =pointerup=. That costs you the legibility this
example has, where every frame's behaviour is spelled at the element. Reach for
it on a measurement, not on principle.
...
resumes. So =[:event/prevent-default]= has to be a client effect, and it has to
...
column width, here — and keep re-asserting it across any morph landing in the
gap. The resizer gets away with it because the drag writes the same property the
server's render will write, and a morph mid-drag is rare enough to look like a
flicker rather than a bug. Offworld has places to hold a client-owned value
(=data-ignore-morph=, Replicant's per-node memory) and no vocabulary at all for
the /handoff/: when the client may stop asserting. That is hand-rolled per
gesture today, and it is the one part of a rich seam this design doesn't make
legible.
