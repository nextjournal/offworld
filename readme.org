#+TITLE: 🪐 Offworld #+OPTIONS: toc:2 num:nil
...
and 🌏server. Represent the computation as a set of pure-data actions,
and tag each action by the world it runs in.  Together, the world-tag and
the nexus handler-type indicate the stage where it runs.*
...
#+begin_src sh
bb dev # shadow-cljs watch + http-kit on :8000 + clerk on :9001 bb
test-clj # jvm tests bb test-cljs # node tests via shadow
#+end_src
...
The library is eight files under a thousand lines: =offworld.cljc=,
=stem.cljc=, =staging.cljc=, =order.cljc=, =guard.cljc=, =inline.cljc=,
=util.cljc=, =nexus/registry.cljc=.  Everything under =demo/= is the demo.
...
That handler is a *closure*: a function bundled together with the
bindings it captured from where it was written. In a browser-only app
this is fine, because the function is created and called in the same
place.
...
applies it to =:winter=. Nothing crossed the wire but a keyword
and a value.

The way we rewrote this code is a standard technique, called
*defunctionalization*. It's worth knowing, because once you see it you
see it everywhere. Reynolds described it in 1972 as a compiler
technique: replace a function value with a tag plus the values it
captured, and invoke dispatch procedure to apply the tag to values.

In our case, isn't an optional style choice.  Any framework that runs
...
- A *place* :: a signal, a web-component attr, a named cell in the
  markup whose meaning lives in a
callback registered somewhere else
...
data-as-a-value admits that the others don't:

- *It crosses.* A closure can't be split across a wire. An action,
  though, is a vector whose first element names its handler. a lookup
  tells you what world it belongs to, at any expansion depth. Without
  this there's no hybrid mode at all; the rest of the list is a bonus.
...
- *It can be checked before it runs.* Stranded client references and
  unregistered heads fall out of walking the same structure,
  statically, without executing anything.
- *It can be shown.* Every key classified by world and kind, coloured
  in the editor, read out on hover. An intent prints at its call site
  with =#p=.
- *It can be extended by a caller who doesn't understand it.* A layer
  that knows nothing about a component's concern can still =into= or
  =mapv= the intent it was handed, so components compose without
  either layer naming the other's business.
...
Two more are reachable and unbuilt: authorizing a dispatch by its
/shape/ before anything runs, and compiling a statically-known
dispatch to direct client code.
...
#+begin_src clojure
(defn randomize-button [{::🌿/keys [stem]}] [:button {:on {:click
...
=(get-season stem)= is an ordinary function call. It runs /now/, and
what lands in the vector is the value
=:spring=. =[:event/key-modifiers]= is not a call. It's a vector and
it stays one, naming a value that doesn't exist yet because the click
hasn't happened.
...
There /is/ string interpolation here, but as dumb transport rather
than code generation: one opaque blob in one fixed slot, beside a
dispatch URL constant for the whole app. The JavaScript around it is
byte-identical on every element, and no part of what an action /means/
is ever assembled into it.

Compare that against the common pattern where each seam hand-writes
its own attribute expression, so every interaction invents a little
language only the browser can read.

** In the browser

The click fires. =divert= decodes the payload and dispatches it
against a /client view/ of the registry — one whose placeholders are
filtered to the client-marked ones. Each action is then /interpolated/
— walked, with every placeholder replaced by the value it stands for —
as Nexus reaches it, so =[:event/key-modifiers]= resolves against the
real DOM event and =[[::randomize [:shift nil nil] :spring]]= is what
expands.

=::randomize= is client-marked, so it expands here:

#+begin_src clojure
(nxr/register-action! ::randomize ^::🪐/client (fn [_ key-mods season]
  (let [path [::path :to :season] reset?  (contains? (into #{}
  key-mods) :shift) rand-season (first (rand-nth (seq (dissoc
  season->holiday season))))] (if reset?  [[:browser/alert "Holiday
  season has been reset."]  [:effects/save path :spring]]
...
The two effects it returns don't live in the same
place. =:browser/alert= is client-marked and runs
immediately. =:effects/save= is =^::🪐/server=, so an interceptor at
Nexus's =:before-action= phase recognises the tag, sets the action
aside for the server, and empties the rest of that action's queue so
nothing runs it locally.

That phase is why the split is trustworthy. It fires for /every/
action Nexus reaches, at any expansion depth — a server effect
returned from inside a client expansion three levels down is caught
exactly like one written at the top of the vector. The boundary is not
a filter over the dispatch you authored; it is a rule applied to every
action the dispatch produces. And the action is interpolated against
the client view before it's set aside, so a client-stage value
resolves before it crosses while a server-stage placeholder crosses
untouched.
...
the library's business, and there are several defensible answers: push
on every state change, push on a fixed interval only when the render
differs, throttle to a frame, re-render only the subtree that
changed. The demo takes the first — a watch on the system atom
re-renders the whole UI, runs =replicant->d*= over it, renders it to a
string, and pushes it down the SSE connection as a
...
One vector, one authoring site, six stages, four of them in different
runtimes, and the boundary between them marked by a single piece of
metadata on a handler — not by a rewrite, not by a second language,
not by a naming convention.

In CSR mode the same code runs with no split: the client view isn't
applied, the interceptor isn't present, and every action runs where it
...
{:on {:scroll ^{:datastar/modifiers [:throttle.100ms]} [[::ng/scroll
              path [:event.target/scroll-top]
...
#+begin_src clojure
(nxr/register-action! ::ng/scroll ^::🪐/server (fn [_ path top left]
  [[:effects/save (concat path [:scroll-top]) top] [:effects/save
  (concat path [:scroll-left]) left]]))
#+end_src

(=register-expansion!= is Nexus's newer name for the same
call. "Action" is the data; "expansion" is the handler that turns one
into more.)
...
control flow as data rather than as code.
...
#+begin_src clojure
{:node/focus (fn [ctx _ node] (.focus node)) :event/prevent-default
 #(.preventDefault (get-evt %)) :effects/save ^:nexus/batch
 ^::🪐/server (fn [_ system path-vs] (swap! system (fn [state] (reduce
 (fn [acc [path v]] (assoc-in acc path v)) state path-vs))))}
#+end_src
...
{:event.target/value #(some-> % get-evt .-target .-value)
 :event/key-modifiers (fn [dd] (let [e (:replicant/dom-event dd)]
 [(when (.-shiftKey e) :shift) (when (.-altKey e) :alt) (when
 (.-ctrlKey e) :ctrl)]))}
#+end_src
...
(nextjournal.offworld.nexus.registry/register-many!
 {:nexus/system->state deref :nexus/effects {...}  :nexus/placeholders
 {...}})
#+end_src
...
#+begin_src clojure
(nxr/register-placeholder! ::enter?  (fn [dd] (= "Enter" (.-key
  (:replicant/dom-event dd)))))

{:on {:keydown [[::🚦/guard [::enter?]  [[::commit
                 [:event.target/value]]]]]}}
#+end_src
...
#+begin_src clojure
(nxr/register-placeholder! ::alt-held?  (fn [dd held?] (= held?
  (.-altKey (:replicant/dom-event dd)))))

(defn button [{:keys [label click-ax alt-click-ax]}] [:button {:on
  {:click [[::🚦/guard [::alt-held? true] alt-click-ax] [::🚦/guard
  [::alt-held? false] click-ax]]}} label])
#+end_src
...
straight in with no wrapping. Require =nextjournal.offworld.guard= from
code the client bundle loads, or the registration never runs there.
...
The law's other half: /each value in an intent picks its own stage/, and
the choice is made by how you write the argument, not by a setting.
...
coordinate into immutable history* — Datomic's =basis-t= — and re-read
=as-of= it at dispatch, retaining nothing; *stash it server-side and
carry a claim-check token*; or *hand over the closure*, everything pinned
...
- =:stranded-client-ref= — a client-stage reference that has reached a
  server stage. It resolves only in the browser, every client stage is
  already past, and the context its resolver needs is gone.
...
Nexus's =:before-action= phase and sees every action Nexus reaches, at
any expansion depth, in whichever world it is installed in. An action
that an expansion computed, a caller injected, or a guard nested is by
then an ordinary action like any other, and is checked like one.
...
break it? re-frame answers with tooling, and re-frame-10x is a visual
...
#+begin_src clojure
(defn main-view [state] (for [k [:dresden :hanover]] (station-panel
  (🌿/+ state [:stations k] {:station k}))))
#+end_src
...
(defn get-day {::🌿/deps #{`get-season}} [stem] (season->holiday
  (get-season stem)))
#+end_src

Getters are just functions of the stem. =::🌿/deps= is a seed for
dependency tracing — =🌿/q= and =🌿/trace= record a call tree at runtime —
but the static registry it's aiming at isn't finished. Treat =deps= as
documentation today.
...
#+begin_src clojure
[[:node/hide-popover [::🌿/el popover-id]] [:node/focus [::🌿/el
 choice-id]]]
#+end_src
...
Replicant hands the DOM node to placeholder- and action-handlers, and
...
#+begin_src clojure
[:div {:id id :data-ignore-morph true :replicant/on-mount [[::init id
       {:center center}]]}]

[:button {:on {:click [[::pan-to id 4.89 52.38] [:effects/save path
                       {:center [4.89 52.38]}]]}} "Amsterdam"]
#+end_src
...
The DOM event system fuses three unrelated jobs:
...
against a direct call it buys:

- late binding
- zero-or-many callees
- error isolation
- name-decoupling (not time-decoupling)
...
React built a second propagation path that follows
...
#+begin_src clojure
"Enter" [[:event/prevent-default] [:node/hide-popover [::🌿/el
         popover-id]] [:node/blur [::🌿/el anchor-id]]
         [:node/set-checked [::🌿/el choice-id] true] [:effects/conj
         (conj path :filters) (first filters-to-add) #{}]]
#+end_src
...
explicitness.  That's why we recommend per-element event listeners;
only delegate when there's a meaningful performance advantage. Please
avoid premature optimization. Barring certain domains, 90% of all UI
procedures are simply /fast enough/.

** Per-frame work

*An intent is one per decision, not one per sample.* Nexus should start and stop a
gesture; it should never see a =pointermove=.

The reason is specific to SSR and it isn't a matter of taste: there, a dispatch
that reaches the server is an HTTP request. A drag handler that dispatches every
frame is sixty requests a second, each encoding a payload, waking the server's
dispatch loop and producing a render. The per-frame work — the drag preview, the
pointer capture, the style writes — belongs in a client effect owning its own
=requestAnimationFrame= loop, started by one intent and torn down by another. Two
dispatches per gesture rather than a hundred.

In CSR that cost is gone and the shape still holds, because the same code runs in
both worlds.

*=preventDefault= is a separate constraint, and a sharper one.* The browser
honours it only while it is dispatching the event — inside the handler it called,
in the same task. That window is not one of Offworld's stages; it is the browser
mechanism that /starts/ them, and it has closed by the time anything asynchronous
resumes. So =[:event/prevent-default]= has to be a client effect, and it has to be
reached before the dispatch yields.

Nexus makes that reachable, because it interleaves interpolation and effects
rather than resolving a whole intent up front: each action is interpolated as it
is reached, and its effect runs before the next action is looked at. Put
=[:event/prevent-default]= first and it runs first, before the placeholders in
...
#+begin_src clojure
(add-watch system ::ui/render (fn [_ _ _ new-state]
  (broadcast-elements!  (sse-message {:event "datastar-patch-elements"
...
#+begin_src clojure
(r/set-dispatch!  (fn [dispatch-data actions] (if js/navigator.onLine
 (nxr/dispatch system dispatch-data actions) (🌠/offline-dispatch
...
#+begin_src clojure
  (update nexus :nexus/interceptors
          conj (📈/proposing !counters))   ; client
  (update nexus :nexus/interceptors
          conj (📈/checking !actor-state)) ; server
#+end_src
...
at it — an inline =def=, or hashp's =#p=, anywhere in the composition
chain:

#+begin_src clojure
[:button {:on {:click #p [[::randomize [:event/key-modifiers]
                           (get-season stem)]]}} "Randomize
...
** Are we smelling Greenspun's tenth rule?
...
and the staging law answers it by narrowing that case to one: eliding
a round trip.

** Why not infer the client/server boundary with macros?
Hyper and Electric Clojure do versions of this and buy real
ergonomics. What they don't leave you with is a value — so nothing to
walk, check, colour or log, and a behaviour change that shows up in
review as an altered string.

** Isn't shipping data slower than shipping code?
Two costs get confused. Measured on a 10,000-row table: on the /wire/,
compression handles it — the markup came to around 200KB Brotli'd, and
replacing each cell's inline intent with a shared template or an
opaque server-held token bought about 12% more. On /parse/, neither
move helped at all, because the browser decompresses back to full text
and parse time tracks raw bytes. The opaque-reference alternative is
the smaller one, not the faster one. And the data form is a source,
not an obligation: a statically-known dispatch could in principle
compile to direct client code, at the cost of two implementations that
must be proven to agree. Not built.
...
whole store. re-frame put it more bluntly a decade ago: "Please, just
...
- <<<Nexus>>> :: a table of named handlers, and a loop that drains a list of
  things to do by looking each one up as it goes.
- <<<Datastar>>> :: attribute-driven client behaviour, and DOM /morphing/ over
  SSE: patching a live DOM tree in place to match a new HTML fragment,
  pushed down a long-lived HTTP response the server keeps writing to.
...
- <<<staged computation>>> :: an interaction written as one value whose
  ...
  runs all at once.
- <<<stage>>> :: one link in a staged computation:
  morph, client-dispatch, dispatch-request, server-dispatch and finally render.
...
- [[https://developer.salesforce.com/docs/platform/lwc/guide/events-propagation.html][LWC: event propagation]] and [[https://developer.salesforce.com/blogs/2021/08/how-events-bubble-in-lightning-web-components][How events bubble in LWC]] — the clearest
  statement of what bubbling costs a component's API
- [[https://blog.angular-university.io/angular-component-design-how-to-avoid-custom-event-bubbling-and-extraneous-properties-in-the-local-component-tree/][Angular: avoiding custom event bubbling]] — the mirror-image problem,
  and why a framework without cheap threading chooses a different
  implicit channel
- [[https://dev.to/open-wc/composed-true-considered-harmful-5g59][composed: true considered harmful?]] — Westbrook Johnson, 2019
- [[https://pm.dartus.fr/posts/2021/shadow-dom-and-event-propagation/][Shadow DOM and event propagation]] — retargeting, and =composedPath()=
  as an "escape hatch to the shadow DOM encapsulation model"
- [[https://blog.logrocket.com/deep-internals-event-delegation/][Internals of event delegation]]
- [[https://dl.acm.org/doi/10.1145/800194.805852][Reynolds: Definitional Interpreters for Higher-Order Programming
  Languages]] (1972) — where defunctionalization is introduced
- [[https://arxiv.org/pdf/2111.10413][CPS, defunctionalization, accumulations and associativity]] — Danvy et
  al.
- [[https://www.w3.org/2001/tag/doc/leastPower.html][Berners-Lee & Mendelsohn: the rule of least power]] — why the weaker
  notation is the one you can analyse, which is the rule the staging
  ladder follows
- [[https://www.itu.dk/people/sestoft/pebook/][Jones, Gomard & Sestoft: Partial Evaluation]] — binding-time analysis,
  the static / dynamic split that [[*Early or late, per value][Early or late, per value]] is an
  informal version of
- [[https://docs.datomic.com/reference/filters.html][Datomic: database filters]] — =as-of= and =basis-t=, the
  coordinate-into-history that makes rung 2 of that ladder a long
  instead of a snapshot store
- [[https://www.enterpriseintegrationpatterns.com/patterns/messaging/StoreInLibrary.html][Enterprise Integration Patterns: Claim Check]] — store the payload,
  pass a token, redeem it later; rung 3
