...
Offworld tries to prioritize the user experience by being intent-driven.
An intent is one thing, at one place. Its inputs and outputs are fully
legible - legible where you express it, and legible wherever it goes.
By "it goes," I mean: staged computations.

Because intent is a staged computation, we model intent as a defunctionalized continuation.

Intent is staged because it relies on values and effects from the browser.

Only the browser can know and do them, not the server.

On the other hand, rendering UI on the server has benefits[citation]. We want to do it.

So, what can we do? We render these client-bound inputs & outputs as
data-structures. To borrow nexus' vocabulary,
an input is a "placeholder", and an output is a client "action".
When the user clicks the button, they commit the intent.
The client "dispatches" the intent - first it resolves the placeholders,
then 

will resolve these when it dispatches the user inten


the client to resolve when it dispatches the intent. Similarly, we render
an "action" for any side-effects the client should do. Placeholders,
actions, and the intent that joins them are all fully legible to any process,
any time, anywhere.
...
* Inline expressions.
Some devs feel the registry pattern is too much ceremony - naming everything, lots of
indirection, and maybe the "abstract machine" just distracts from what's really happening.

They just want a render-fn that's "readable" in the sense that what it "does"
is inlined. In other words, the counter literally says `(swap! count inc)`
somewhere in the render-fn's body.

Transparency is reduced, because the client only sees an opaque key, not a server-action.
The server-bound consequence of the intent is invisible. On the other hand,
the client-bound inputs and outputs are fully visible, in normative form.

Offworld offers a way to opt into this end of the the transparency/power tradeoff.

Devs can couple place with behavior, by deriving a cursor from the stem.

Devs can invoke a macro, express a mutation inline, and offworld will "compile"
the resulting continuation out of that. The intent stays anonymous. Instead of
belonging to the static nexus registry, the server caches it dynamically.
With an eviction policy that respects the canonical form of the intent (content-addressed?).

#+begin_src clojure

  (defn save-local [state value]
    [[:fx/save (🌿/path state) value]])

  #?(:cljs )

  (defmacro act!
    "register a server-side closure under a gensymmed key.
       return [:invoke :xyz-gensymmed-key]
       use the same eviction strategy as we use for feather etc.
       walk & collect inline uses of `client!`. for each use,
       if its a key-or-vector, put that placeholder vector in the  
  either look up a registered placeholder, or do the body as a compiled squint expr,
       placed in the action vector:

       [:invoke :xyz-gensymmed-key {:client-expr1 [::color]
                                    :client-expr2 'compiled.squint.expr()']]
      "
    [])

  (defn blue-button [state]
    [:button {:on
              {:click
               [(server! (let [blue? (= :blue (client! ::color))
                               next-val (if blue? :foo :bar)]
                           (client! (alert! "changed color"))
                           (save-local state next-val)))]}}
     (🌿/local state)])

  (defn inc-button [state]
    (let [*ct (cursor state)]
      [:button {:on {:click [(server! (swap! *ct inc))]}}])) ;; whole closure claim-checked

  (defn inc-button [state]
    (let [*ct (cursor state)]
      [:button {:on {:click [(server! (swap! *ct inc))]}}]))
  
  (defn inc-button [state]
    (let [*ct (cursor state)]
      [:button {:on {:click [(server! (when (client! document.-someState) ;; placeholder
                                        (swap! *ct inc)))]}}]))

  (defn inc-button [state]
    (let [*ct (cursor state)
          x true]
      [:button {:on {:click [(server! (when (client! (or (server-value x) ;; server-rendered value
                                                         document.-someState)) ;; placeholder
                                        (swap! *ct inc)))]}}]))

  (defn inc-button [state]
    (let [*ct    (cursor state)
          x      (some-big-query datomic/db)
          maybe? (some-pred x)]
      [:button {:on {:click [(server! (when (client! (or (server-claimed (maybe? x)) ;; server-claim-checked closure
                                                         document.-someState)) ;; placeholder
                                        (swap! *ct inc)))]}}]))
#+end_src
...
*** What a caller injects, and what it may be.

Injection is render-time function application, so what a caller hands a
render-fn is whatever that application needs. =nextjournal.offworld/inject=
names the three shapes worth accepting, and a render-fn calls it wherever a
caller decides what happens:

#+begin_src clojure
(defn table [{:keys [on-scroll on-sort on-resize row-container-path row-height]}]
  ;; ...
  {:on {:scroll (🪐/inject on-scroll {:row-offset [::row-offset row-container-path row-height]
                                      :row-limit  [::row-limit row-container-path row-height]
                                      :sort       sort-spec})}})
#+end_src

A *vector* is a dispatch the caller wrote out in full; nothing is injected
into it, and it composes with the render-fn's own actions by ordinary
=into=. This is the right shape when the caller can say the whole thing —
a resize handle's preview and commit, where =[:event/client-x]= is the only
late value and the caller can write that name itself:

#+begin_src clojure
(resizer {:on-resize [[::resize-preview [:event/client-x] [::🌿/el header-path] widths]]
          :on-commit [[::resize-commit  width-path [:event/client-x] [::🌿/el header-path]]]})
#+end_src

A *function* is called with the params and returns the dispatch. It is the
shape that says at the call site "you will be handed something", which a
bare vector cannot say — so if a render-fn ever wants to append arguments to
a caller's action, it should take a function instead and let the caller do
the appending. Nothing should quietly =conj= onto a vector that reads as
finished where it was written.

A *keyword* names an action the caller registered, called with the params as
its single map argument. Usually the one to reach for: one definition site,
greppable, no closure, and the render-fn can add a key to what it injects
without breaking any handler.

#+begin_src clojure
(nxr/register-action! ::refetch ^::🪐/server
  (fn [state {:keys [row-offset row-limit sort]}]
    [[::λ/fetch (merge (λ/get-params state)
                       {:offset row-offset :limit row-limit}
                       (when (seq sort) {:sort sort}))]]))

(table {:on-scroll ::refetch :on-sort ::refetch})
#+end_src

**** Which shape is available is a staging question, not a taste one.

The handler runs in a world, and the world it needs decides what it may be.
=::refetch= above is =^::🪐/server= because it reads state — a client-world
handler has no stem to read. But a handler that /emits/ a client-world
placeholder must not be server-marked, or the placeholder arrives past
=request= as inert data, which is the =:stranded-client-ref= violation:

#+begin_src clojure
;; client world: emits a placeholder that resolves before the request
(nxr/register-action! ::refetch-fresh
  (fn [_ {:keys [row-limit]}]
    [[::λ/fetch {:limit row-limit :selected-columns [::columns-from-localstorage]}]]))
#+end_src

Same seam, same table, two handlers in different worlds — one because it
consumes server state, one because it produces a client reference. Reading
forces server; emitting a client reference forbids it.
...
* Case studies
** Treasure Hunt
** Ceremony vs. inlining
Offworld gives you what a cursor does, where placeful programming makes sense: the element.
* Objections
